...
class AppStore extends Component {
  ...
  render() {
    ...
    const currentTabApps = appsList.filter(
      item => item.category === currentTabId,
    )
    const filteredInputApp = currentTabApps.filter(item =>
    ...
    return (
      <div className="main-container">
        ...
        <ul className="app-list">
          {filteredInputApp.map(item => (
            <AppItem key={item.id} appData={item} />
          ))}
        </ul>
      </div>
    )
